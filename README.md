...
| 专业岗位用户 | 使用专业场景应用（审标/比标/规范问答等）、管理任务文件、查看结构化结果 |
...
| AI 投标 | 读标 / 写标 / 比标 / 清标四个子应用 | **比标、读标已接通真实链路**；写标/清标按规划推进 |
| 规范问答 | 标准/规范检索、阅读与 AI 对话（DocViewer） | 原型 |
| AI 配音 | 配音任务与音色管理 | 原型 |
| API 管理 | 开放接口查看/调试 | 原型 |
| 个人中心 | 用户资料与偏好 | 原型 |
...
graph TB
    subgraph FE[前端层]
        U["user-web 用户端<br/>工作台 · 应用广场 · AI 投标 · 规范问答 · AI 配音 · API 管理"]
...
> 图例：`==>` 表示已打通真实链路的调用（当前为比标、读标纵向切片）；`-->` / `-.->` 为常规依赖与规划路径。业务模块按「应用清单」装配，除比标、读标外当前均为前端原型，后端随规划逐步实现。

### 模块结构

| 目录 | 说明 |
|---|---|
| `backend/DredgeAI.BidCompare` | ABP（.NET 8）主服务：身份/权限（OpenIddict）、应用清单装配、任务与后台作业、存储抽象；当前已实现比标模块（状态机、AnGIneer 解析编排、算法调用、AI 分析、报告导出）与读标模块（招标文件解析、基准库抽取与人工确认） |
...
| angineer-ai-inference | Python 库（v0.2.2） | ai-gateway 依赖 | LLM 推理库：多模型路由 / 重试 / 熔断 / SSE，ai-gateway 包装为 OpenAI 兼容接口 |
