...
| Forget to add using statement for `IShiwDbContextHandler` | Add `using Shiw.Abp.BaseEntityFrameworkCore;` when using `IShiwDbContextHandler` |
...
public class MyProjectDbContext : AbpDbContext<MyProjectDbContext>
{
    ...
    public MyProjectDbContext(DbContextOptions<MyProjectDbContext> options, IShiwDbContextHandler handler)
        : base(options)
    {
        _handler = handler;
    }
    ...
    protected override void OnModelCreating(ModelBuilder builder)
    {
        ...
        builder.ConfigureMyProject(_handler);
    }
}
```
...
{
    public static void ConfigureMyProject(this ModelBuilder builder, IShiwDbContextHandler handler)
    {
        ...
        {
            b.ToTable(
                handler.TableNameHandler($"{MyProjectConsts.DbTablePrefix}{nameof(Book)}"), MyProjectConsts.DbSchema);
            b.ConfigureByConvention(handler); // ABP conventions (audit, soft-delete, etc.)

            // Property configurations
            b.Property(x => x.Name)
                .HasColumnName(handler.FieldNameHandler(nameof(Book.Name)))
                .IsRequired()
                .HasMaxLength(BookConsts.MaxNameLength);

            b.Property(x => x.Price)
                .HasColumnName(handler.FieldNameHandler(nameof(Book.Price)))
...
> **Note**: Before using `ShiwBaseEntityFrameworkCoreModule`, you need to install the `Shiw.Abp.BaseEntityFrameworkCore` package first.
